...
<body class="bg-gray-100">
    <div class="max-w-3xl mx-auto p-6">
        <div class="bg-white rounded-xl shadow p-6">
            <h1 class="text-2xl font-bold mb-6">Edit Data Keluarga</h1>

            <form action="{{ route('admin.keluarga.update', $keluarga->id) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')

                <div>
                    <label class="block mb-1 font-medium">No KK</label>
                    <input type="number" name="no_kk" value="{{ old('no_kk', $keluarga->no_kk) }}"
                ...
                <div>
                    ...
                    <textarea name="alamat" rows="4"
                              class="w-full border rounded-lg px-3 py-2">{{ old('alamat', $keluarga->alamat) }}</textarea>
                    @error('alamat')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="kode_pos" value="{{ old('kode_pos', $keluarga->kode_pos) }}"
                               class="w-full border rounded-lg px-3 py-2">
                        @error('kode_pos')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <select name="kepala_keluarga_id" class="w-full border rounded-lg px-3 py-2">
                        ...
                        @foreach($wargas as $warga)
                            <option value="{{ $warga->id }}"
                                {{ old('kepala_keluarga_id', $keluarga->kepala_keluarga_id) == $warga->id ? 'selected' : '' }}>
                                {{ $warga->nama_lengkap }}
                            </option>
                        @endforeach
                    </select>
