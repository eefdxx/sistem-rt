...
<head>
    ...
    <title>Modifikasi Warga - {{ $warga->nama_lengkap }}</title>
...
<body class="bg-slate-50 antialiased text-slate-800 selection:bg-indigo-500 selection:text-white">
    <div class="min-h-screen">
        ...
        <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            <div class="mb-8">
                ...
                <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Mutasi Data Warga</h1>
                <p class="text-slate-500 font-medium mt-1">Perbarui informasi kependudukan untuk warga <span class="text-indigo-600 font-bold">{{ $warga->nama_lengkap }}</span>.</p>
            </div>
            ...
            <form action="{{ route('admin.warga.update', $warga->id) }}" method="POST" class="space-y-8">
                ...
                <div class="bg-white rounded-[2rem] border border-slate-200/60 shadow-xl shadow-slate-200/40 p-8 transition-all hover:shadow-2xl hover:shadow-slate-200/50">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="nik" value="{{ old('nik', $warga->nik) }}" required
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <label class="relative flex items-center justify-center p-3.5 rounded-2xl cursor-pointer transition-all group {{ $warga->jenis_kelamin == 'Laki-laki' ? 'bg-white ring-2 ring-indigo-500' : 'bg-slate-50 ring-1 ring-slate-200 hover:bg-white' }}">
                                    <input type="radio" name="jenis_kelamin" value="Laki-laki" class="sr-only" {{ old('jenis_kelamin', $warga->jenis_kelamin) == 'Laki-laki' ? 'checked' : '' }}>
                                    <span class="text-sm font-bold {{ $warga->jenis_kelamin == 'Laki-laki' ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-600' }}">Laki-laki</span>
                                </label>
                                <label class="relative flex items-center justify-center p-3.5 rounded-2xl cursor-pointer transition-all group {{ $warga->jenis_kelamin == 'Perempuan' ? 'bg-white ring-2 ring-pink-500' : 'bg-slate-50 ring-1 ring-slate-200 hover:bg-white' }}">
                                    <input type="radio" name="jenis_kelamin" value="Perempuan" class="sr-only" {{ old('jenis_kelamin', $warga->jenis_kelamin) == 'Perempuan' ? 'checked' : '' }}>
                                    <span class="text-sm font-bold {{ $warga->jenis_kelamin == 'Perempuan' ? 'text-pink-600' : 'text-slate-400 group-hover:text-pink-600' }}">Perempuan</span>
                                </label>
                            </div>
                ...
                <div class="bg-white rounded-[2rem] border border-slate-200/60 shadow-xl shadow-slate-200/40 p-8 transition-all hover:shadow-2xl hover:shadow-slate-200/50">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2 md:col-span-2">
                            <label class="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Ganti Kartu Keluarga (KK)</label>
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="status_warga" required class="w-full px-5 py-3.5 rounded-2xl bg-slate-50 border-none ring-1 ring-slate-200 focus:ring-2 focus:ring-indigo-500 transition-all font-medium appearance-none">
                                <option value="warga_tetap" {{ old('status_warga', $warga->status_warga) == 'warga_tetap' ? 'selected' : '' }}>Warga Tetap</option>
                                <option value="kontrak" {{ old('status_warga', $warga->status_warga) == 'kontrak' ? 'selected' : '' }}>Kontrak / Kos</option>
    ...
    <script>
        // Radio UI Enhancement logic (identical to create view for consistency)
        document.querySelectorAll('input[type="radio"]').forEach(radio => {
            radio.addEventListener('change', function() {
                document.querySelectorAll('input[name="'+this.name+'"]').forEach(r => {
                    r.parentElement.classList.remove('ring-indigo-500', 'ring-pink-500', 'bg-white', 'ring-2');
                    r.parentElement.classList.add('bg-slate-50', 'ring-1', 'ring-slate-200');
                    r.parentElement.querySelector('span').classList.remove('text-indigo-600', 'text-pink-600');
                    r.parentElement.querySelector('span').classList.add('text-slate-400');
                });
                if(this.checked) {
                    this.parentElement.classList.remove('bg-slate-50', 'ring-1', 'ring-slate-200');
                    ...
                    this.parentElement.querySelector('span').classList.remove('text-slate-400');
