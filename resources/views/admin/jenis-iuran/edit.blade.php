...
<head>
    ...
    <title>Edit Jenis Iuran - Admin Panel</title>
...
<div class="min-h-screen">
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="mb-8">
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Edit Jenis Iuran</h1>
            <p class="text-slate-500 text-sm mt-1">Perbarui informasi jenis iuran. Perubahan tidak akan mempengaruhi tagihan yang sudah diterbitkan sebelumnya.</p>
        </div>

        <div class="bg-white rounded-3xl border border-slate-200/60 shadow-xl p-8">
            <form action="{{ route('admin.jenis-iuran.update', $jenis_iuran->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="nominal_default" value="{{ old('nominal_default', $jenis_iuran->nominal_default) }}" required min="0" step="1000"
                    ...
                    <div>
                        ...
                        <select name="periode" required
                            ...
                            <option value="insidental" {{ old('periode', $jenis_iuran->periode) === 'insidental' ? 'selected' : '' }}>Insidental (Sewaktu-waktu)</option>
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="deskripsi" rows="3"
                            ...
                            class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none">{{ old('deskripsi', $jenis_iuran->deskripsi) }}</textarea>
                    </div>

                    {{-- Status Aktif --}}
                    <div class="md:col-span-2 flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-2xl p-4">
                        <input type="hidden" name="is_active" value="0">
                        <input type="checkbox" name="is_active" value="1" id="is_active" {{ old('is_active', $jenis_iuran->is_active) ? 'checked' : '' }}
                ...
                <div class="pt-6 flex gap-3">
                    ...
                    <button type="submit" class="flex-1 py-3.5 rounded-2xl bg-indigo-600 text-white font-black text-sm hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-600/20">Perbarui Jenis Iuran</button>
                </div>
            </form>
        </div>
    </main>
